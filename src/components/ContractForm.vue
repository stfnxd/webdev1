<template>
    <section class="contractForm col-5">
        ...
        <section>
            ...
            <span v-if="!isFormValid().customerType" class="warning"><font-awesome-icon icon="circle-exclamation" /> Kunde
                typen skal vælges</span>
            ...
            <span v-if="!isFormValid().contractType" class="warning"><font-awesome-icon icon="circle-exclamation" />
                Kontrakttypen skal vælges</span>
            ...
            <div
                ...
                <div v-show="formData.customer.customerType != 'Split' && formData.customer.contractType != 'Stilstand'"
                    ...
                    <input v-model="formData.customer.season" @input="emitValue" type="checkbox" id="checkboxSeason"
                        name="checkbox">
                </div>
            </div>
            ...
            <div v-show="formData.customer.contractType == 'Nytegning'" class="contract-checkbox">
                ...
                <input v-model="formData.customer.import" @input="emitValue" type="checkbox" id="checkboxImport"
                    name="checkbox">
            </div>
        </section>
        ...
        <section>
            ...
            <div v-show="formData.customer.contractType == 'Genleasing'">
                ...
                <input v-model="formData.customerId" @input="emitValue" type="text" id="customerId" class="form-control"
                    name="customerId" placeholder="Indtast kundens id">
                <button @click="fetchCustomerData">Søg</button>
            </div>

            <label for="name">Navn</label>
            <input v-model="formData.customer.name" @input="emitValue" type="text" id="name" class="form-control"
                name="name" placeholder="Indtast kundens navn">
            <!-- INPUT VALIDERING: Tjekker om kontrakttypen er valgt -->
            <span v-if="!isFormValid().customerName" class="warning"><font-awesome-icon icon="circle-exclamation" /> Kunde
                navn skal udfyldes</span>

            <label for="email">Email</label>
            <input v-model="formData.customer.email" @input="emitValue" type="email" id="email" class="form-control"
                name="email" placeholder="Indtast kundens email adresse">
        ...
        <section>
            ...
            <div v-show="formData.customer.contractType != 'Stilstand' && formData.customer.contractType != ''"
                class="contract-checkbox">
                <label for="checkboxLevyPaid">Fuld afgift betalt</label>
                <input v-model="formData.vehicle.levyPaid" @input="emitValue" type="checkbox" id="checkboxLevyPaid"
                    name="checkbox" value="1">
            </div>

            <div class="contract-checkbox">
                <label for="checkboxVATDeath">Momsdød</label>
                <input v-model="formData.vehicle.vatDeath" @input="emitValue" type="checkbox" id="checkboxVATDeath"
                    name="checkbox" value="1">
            </div>
            ...
            <span v-if="!isFormValid().vehicleType" class="warning"><font-awesome-icon icon="circle-exclamation" /> Type
                køretøj skal vælges</span>

            <label for="vehicle">Køretøj</label>
            <input v-model="formData.vehicle.vehicle" @input="emitValue" type="text" id="vehicle" class="form-control"
                name="vehicle" placeholder="Indtast køretøj">
            <!-- INPUT VALIDERING: Tjekker om Køretøj er indtastet -->
            <span v-if="!isFormValid().vehicle" class="warning"><font-awesome-icon icon="circle-exclamation" /> Køretøj skal
                udfyldes</span>

            <label for="framenumber">Stelnummer</label>
            <input v-model="formData.vehicle.frameNumber" @input="emitValue" type="number" id="framenumber"
                class="form-control" name="framenumber" placeholder="Indtast stelnummer">
            ...
            <div v-show="formData.vehicle.newVehicle == false">
                ...
                <input v-model="formData.vehicle.firstRegistrationDate" @input="checkDate" type="date"
                    id="firstRegistrationDate" name="firstRegistrationDate" />
            </div>
            ...
            <div v-show="formData.customer.contractType != 'Stilstand'">
                ...
                <div v-if="formData.customer.contractType == 'Nytegning' && formData.customer.import == true">
                    ...
                    <input v-model="formData.contractValues.salePrice" @input="emitValue" type="number" id="salePrice"
                        class="form-control" name="salePrice" placeholder="Indtast udsalgspris i €">
                    <!-- INPUT VALIDERING: Tjekker om Udsalgspris er indtastet -->
                    <span v-if="!isFormValid().salePrice" class="warning"><font-awesome-icon icon="circle-exclamation" />
                        Udsalgspris i € skal udfyldes</span>
                </div>
                <!-- *Eller vises dette -->
                <div v-else>
                    <label for="salePrice">Udsalgspris</label>
                    <input v-model="formData.contractValues.salePrice" @input="emitValue" type="number" id="salePrice"
                        class="form-control" name="salePrice" placeholder="Indtast udsalgspris">
                    <!-- INPUT VALIDERING: Tjekker om Udsalgspris er indtastet -->
                    <span v-if="!isFormValid().salePrice" class="warning"><font-awesome-icon icon="circle-exclamation" />
                        Udsalgspris skal udfyldes</span>
                </div>
            ...
            <div v-show="formData.customer.customerType == 'Split' || formData.customer.contractType == 'Nytegning'">
                ...
                <div v-if="formData.customer.import == true">
                    ...
                    <input v-model="formData.contractValues.cost" @input="emitValue" type="number" id="cost"
                        class="form-control" name="cost" placeholder="Indtast kostpris i €">
                    <!-- INPUT VALIDERING: Tjekker om Kostpris er indtastet -->
                    <span v-if="!isFormValid().cost" class="warning"><font-awesome-icon icon="circle-exclamation" />
                        Kostpris i € skal udfyldes</span>
                </div>
                <!-- *Ellers vises dette -->
                <div v-else>
                    <label for="cost">Kostpris</label>
                    <input v-model="formData.contractValues.cost" @input="emitValue" type="number" id="cost"
                        class="form-control" name="cost" placeholder="Indtast kostpris">
                    <!-- INPUT VALIDERING: Tjekker om Kostpris er indtastet -->
                    <span v-if="!isFormValid().cost" class="warning"><font-awesome-icon icon="circle-exclamation" />
                        Kostpris skal udfyldes</span>
                </div>
            </div>
            ...
            <div v-show="formData.customer.contractType == 'Genleasing'">
                ...
                <input v-model="formData.contractValues.estimatedMarketValue" @input="emitValue" type="number"
                    id="estimatedMarketValue" class="form-control" name="estimatedMarketValue"
            ...
            <div v-show="formData.customer.contractType == 'Genleasing' || formData.customer.contractType == 'Stilstand'">
                ...
                <span v-if="!isFormValid().residualValue" class="warning"><font-awesome-icon icon="circle-exclamation" />
                    Restværdihæftelse skal udfyldes</span>
            </div>
            ...
            <div v-show="formData.customer.contractType == 'Pristjek'">
                ...
                <input v-model="formData.contractValues.cashPrice" @input="emitValue" type="number" id="cashPrice"
                    class="form-control" name="cashPrice" placeholder="Indtast kontantpris">
                <!-- INPUT VALIDERING: Tjekker om Kontantpris er indtastet -->
                <span v-if="!isFormValid().cashPrice" class="warning"><font-awesome-icon icon="circle-exclamation" />
                    Kontantpris skal udfyldes</span>
            </div>

            <!-- Vises hvis kontrakttypen ikke er "Stilstand" og Fuld afgift ikke er betalt-->
            <div
                v-show="formData.customer.contractType != 'Stilstand' && formData.customer.contractType != '' && formData.vehicle.levyPaid == false">
                <label for="registrationFee">Anslået registreringsafgift</label>
                <input v-model="formData.contractValues.registrationFee" @input="emitValue" type="number"
                    id="registrationFee" class="form-control" name="registrationFee"
            ...
            <div
                v-show="showInitialPrice && formData.customer.customerType != 'Split' && formData.customer.contractType != 'Stilstand'">
        ...
        <section>
            ...
            <input v-model="formData.contractValues.runningTime" @input="emitValue" type="number" id="running-time"
                class="form-control" name="run-time" placeholder="Indtast løbetid i måneder">
            <!-- INPUT VALIDERING: Tjekker om løbetid er indtastet -->
            <span v-if="!isFormValid().runningTime" class="warning"><font-awesome-icon icon="circle-exclamation" /> Løbetid
                skal udfyldes</span>

            <!-- Vises hvis kundetype ikke er false og ikke er split og kontrakttypen ikke er Stilstand-->
            <div
                v-show="formData.customer.season != false && formData.customer.customerType != 'Split' && formData.customer.contractType != 'Stilstand'">
                <label for="active-running-time">Aktiv periode i kontraktens løbetid (i måneder)</label>
                <input v-model="formData.contractValues.activeRunningTime" @input="emitValue" type="number"
                    id="active-running-time" class="form-control" name="active-running-time"
                    placeholder="Indtast aktiv periode i kontraktens løbetid i måneder">
                <!-- INPUT VALIDERING: Tjekker om Aktiv periode er indstastet -->
                <span v-if="!isFormValid().activeRunningTime" class="warning"><font-awesome-icon
                        icon="circle-exclamation" /> Aktiv periode skal udfyldes</span>
            </div>

            <label for="interest-rate">Rente</label>
            <input v-model="formData.contractValues.interestRate" @input="emitValue" type="number" id="interest-rate"
                class="form-control" name="interest-rate" placeholder="8.5%" min="0" max="100">

            <label for="contract-creation">Kontraktoprettelse</label>
            <input v-model="formData.contractValues.contractCreation" @input="emitValue" type="number"
                id="contract-creation" class="form-control" name="contract-creation" placeholder="0">
            ...
            <div v-if="formData.customer.contractType != 'Stilstand'">
                ...
                <input v-model="formData.contractValues.oneTimeBenefit" @input="emitValue" type="number"
                    id="one-time-benefit" class="form-control" name="one-time-benefit" placeholder="20%" min="20" max="30">
            </div>
            <!-- *Ellers vises dette -->
            <div v-else>
                <label for="one-time-benefit">Engangsydelse i procent, ex. moms</label>
                <input v-model="formData.contractValues.oneTimeBenefit" @input="emitValue" type="number"
                    id="one-time-benefit" class="form-control" name="one-time-benefit" placeholder="0%">
            </div>

            <label for="deposit">Depositum ex. moms (i procent)</label>
            <input v-model="formData.contractValues.deposit" @input="emitValue" type="number" id="deposit"
                class="form-control" name="deposit" placeholder="0">

            <label for="depreciation">Afskrivning (anbefalet min. 15% p.a.)</label>
            <input v-model="formData.contractValues.depreciation" @input="emitValue" type="number" id="depreciation"
                class="form-control" name="depreciation" placeholder="Indtast afskrivning i procent">

            <label for="commission">Provision</label>
            <input v-model="formData.contractValues.commission" @input="emitValue" type="number" id="commission"
                class="form-control" name="commission" placeholder="Indtast provision i kroner">
